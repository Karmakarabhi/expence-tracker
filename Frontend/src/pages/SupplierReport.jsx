...
export default function SupplierReport() {
  ...
  const fetchSupplierLedger = async (supplierName) => {
    try {
      const res = await axios.get(`/api/suppliers/ledger/${supplierName}`);
  ...
  const handleRecordSettlement = async (e) => {
    e.preventDefault();
    try {
      await axios.post('/api/suppliers/settle', {
        ...settlementForm,
  ...
  const handleDeleteSettlement = async (settlementId) => {
    if (window.confirm('Delete this settlement record?')) {
      try {
        await axios.delete(`/api/suppliers/settlements/${settlementId}`);
        fetchSupplierLedger(selectedSupplier);
        fetchSuppliers();
      } catch (err) {
